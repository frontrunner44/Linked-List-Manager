...
class LinkedList {
  ...
  // Add a node to the end, redundant because the insertNode can do this with the correct parameter, but I left it in for a method that is explicitly named.
  appendNode(data) {
    this.insertNode(data, this.size+1); // calls the insertNode method to insert the new node to the end of the list
  }

  // Add a node to the front of the list. Redundant because the insertNode can do this with the correct parameter, but I left it in for a method that is explicitly named.
  prependNode(data) {
  ...
  insertNode(data, position) {
    ...
    if(position > this.size+1 || position === 0 || isNaN(position) || data === undefined) { // We use this.size+1 because we want to allow inserting the new node to the end (if the list is 5 long, we can insert 6 to put it in position 6, ie at the end)
      ...
      current = this.getNodeByPosition(position-1); // Grab the position directly BEFORE the position we want to insert the new node so we can place the new node AFTER this node.
